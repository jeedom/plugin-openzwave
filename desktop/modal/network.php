    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <b>{{Remettre à zéro le contrôleur et effacer ses paramètres de configuration réseau.}}</b><br> {{Le
                contrôleur devient un contrôleur primaire, prêt pour ajouter des modules sur un nouveau réseau.}}
                <div class="form-group"><label>{{Veuillez confirmer la remise à zéro en tapant YES}}</label><input
                        class="form-control required" id="confirm_text" data-placement="top" data-trigger="manual"
                        type="text"></div>
    ...
    <div class="container-fluid">
        <div id="content">
            <ul id="tabs_network" class="nav nav-tabs" data-tabs="tabs">
                ...
                <li><a href="#statistics_network" data-toggle="tab"><i class="fa fa-bar-chart"></i> {{Statistiques}}</a>
                </li>
                ...
                <li id="li_state" class="pull-right alert"
                    ...
                    <span style="position:relative; top : -7px;">{{Demande envoyée}}</span></li>
            </ul>
            <div id="network-tab-content" class="tab-content">
                ...
                <div class="tab-pane" id="config_network">
                    <a class="btn btn-success pull-right" id="bt_saveconfCfg"><i class="fa fa-floppy-o"></i>
                        {{Sauvegarder les changements}}</a><br/>
                    <br/>
                ...
                <div class="tab-pane active" id="summary_network">
                    ...
                    <div class="panel panel-primary">
                        ...
                        <div class="panel-body">
                            <p>{{Réseau démarré le}} <span class="network-startTime label label-default"
                                                           style="font-size : 1em;">{{inconnu}}</span> <span
                                    class="network-awakedTime label label-default" style="font-size : 1em;"></span></p>
                            <p>{{Le réseau contient}} <b><span class="network-nodes-count"></span></b> {{noeuds,
                                actuellement}} <b><span class="network-sleeping-nodes-count">{{inconnu}}</span></b>
                                {{dorment}}</p>
                            <p>{{Intervalle des demandes :}} <span class="network-poll-interval label label-default"
                                                                   style="font-size : 1em;">{{inconnu}}</span></p>
                            <p>{{Voisins :}} <span class="network-node-neighbours label label-default"
                                                   style="font-size : 1em;">{{inconnu}}</span></p>
                        </div>
                    </div>
                    ...
                    <div class="panel panel-primary">
                        ...
                        <div class="panel-body">
                            <p><span class="network-state-led">{{inconnu}}</span> {{Etat actuel :}} <span
                                    class="network-state-description label label-default" style="font-size : 1em;">{{inconnu}}</span>
                            </p>
                            <p><span class="network-outgoing-send-queueWarning">{{inconnu}}</span> {{Queue sortante :}}
                    ...
                    <div class="panel panel-primary">
                        ...
                        <div class="panel-body">
                            <p>{{Chemin du contrôleur Z-Wave :}} <span class="network-device-path label label-default"
                                                                       style="font-size : 1em;">{{inconnu}}</span></p>
                            <p>{{Version de la librairie OpenZwave :}}<span
                                    class="network-oz-library-version label label-default" style="font-size : 1em;">{{inconnu}}</span>
                            </p>
                            <p>{{Version de la librairie Python-OpenZwave :}} <span
                                    class="network-poz-library-version label label-default" style="font-size : 1em;">{{inconnu}}</span>
                            </p>
                        </div>
                    </div>
                ...
                <div class="tab-pane" id="actions_network">
                    <table class="table">
                        <tr>
                            <td>
                                <button type="button" id="addDevice" class="btn btn-success"><i
                                        class="fa fa-plus-circle"></i> {{Ajouter module (inclusion)}}
                                </button>
                            </td>
                        ...
                        <tr>
                            <td>
                                <button type="button" id="addDeviceSecure" class="btn btn-warning"><i
                                        class="fa fa-plus-circle"></i> {{Ajouter module en mode sécurisé (inclusion)}}
                                </button>
                            </td>
                            <td>{{Ajouter un nouveau module au réseau Z-Wave en mode sécurisé (peut ne pas marcher si le
                                module ne le supporte pas bien).}}
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <button type="button" id="removeDevice" class="btn btn-danger"><i
                                        class="fa fa-minus-circle"></i> {{Supprimer module (Exclusion)}}
                                </button>
                            </td>
                        ...
                        <tr>
                            <td>
                                <button type="button" id="cancelCommand" class="btn btn-warning"><i
                                        class="fa fa-times"></i> {{Annuler commande}}
                                </button>
                            </td>
                        ...
                        <tr>
                            <td>
                                <button type="button" id="testNetwork" class="btn btn-primary"><i
                                        class="fa fa-check-square-o"></i> {{Test du réseau}}
                                </button>
                            </td>
                        ...
                        <tr>
                            <td>
                                <button type="button" id="healNetwork" class="btn btn-success"><i
                        ...
                        <tr>
                            <td>
                                <button type="button" id="createNewPrimary" class="btn btn-danger"><i
                                        class="fa fa-file"></i> {{Créer un nouveau noeud primaire}}
                                </button>
                            </td>
                            <td>{{Put the target controller into receive configuration mode.\nThe PC Z-Wave Controller
                                must be within 2m of the controller that is being made the primary.. Nécessite SUC.}}
                            </td>
                        </tr>

                        <tr>
                            <td>
                                <button type="button" id="receiveConfiguration" class="btn btn-danger"><i
                                        class="fa fa-file"></i> {{Receive Configuration}}
                                </button>
                            </td>
                            <td>{{Transfering the network configuration from another controller.\n\nPlease bring the
                                other controller within 2m of the PC controller and set it to send its network
                                configuration.}}
                            </td>
                        </tr>
                    ...
                    -->
                        <tr>
                            <td>
                                <button type="button" id="transferPrimaryRole" class="btn btn-primary"><i
                                        class="fa fa-external-link"></i> {{Transférer le rôle primaire}}
                                </button>
                            </td>
                            <td>{{Changer de contrôleur primaire. Le contrôleur primaire existant devient contrôleur
                                secondaire. Please bring the new controller within 2m of the PC controller and set it to
                                receive the network configuration.}}
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td>
                                <button type="button" id="writeconfigfile" class="btn btn-info"><i
                                        class="fa fa-pencil"></i> {{Ecrire le fichier de configuration}}
                                </button>
                            </td>
                        ...
                        <tr>
                            ...
                            <td>{{Redémarrage du contrôleur. Redémarre le contrôleur sans effacer les paramètres de sa
                        ...
                        <tr>
                            ...
                            <td>{{Remise à zéro du contrôleur.}} <b>{{Remet à zéro un contrôleur et efface ses
                                    paramètres de configuration réseau.}}</b><br> {{Le contrôleur devient un contrôleur
                                primaire, prêt pour ajouter de nouveaux modules à un nouveau réseau.}}
                            </td>
                        </tr>
                    </table>
                </div>
