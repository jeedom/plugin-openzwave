...
Si toute fois un module ne peut être inclue en mode sécure, veillez l'inclure en mode *Non sécurisé*.

Une fois en mode inclusion : Jeedom vous le signale.

image:../images/addremove03.png[]

Une fois l'asistant lancé, il faut en faire de même sur votre module (se référer à la documentation de celui-ci pour le passer en mode inclusion).
