...
=== Log
Cette partie permet de choisir le niveau de log ainsi que d'en consulter le contenue.

image:../images/openzwave72.png[]

Sélectionner le niveau puis sauvegarder, le démon sera alors relancer avec les instructions et trace sélectionnée.
...
Une fois le debug terminé, il ne faut pas oublier de retourner sur un niveau moins élevé comme le niveau *Error* qui ne remonte que d'éventuelle erreurs.
...
*** Validation que le réseau ne contient pas d'Ids invalide comme un nodeId 0.
*** Tests des modules secteurs ayant retournés une notification 'Timeout'.
*** Tests si des modules sur piles ne sont pas retourné en sommeil profond après leur cycle de réveil.

[IMPORTANT]
Les tests sanitaires, seront exécuté seulement si le réseau n'est pas solicité. Si des messages sont en attente de traitement, le serveur n'autorisera pas l'éxécution des tests sanitaires.
