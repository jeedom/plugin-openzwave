...
[TIP]
La commande *Reprendre de...* vous permet de reprendre la configuration d'un autre module identique, sur le module en cours.
...
Si aucun fichier de configuration n'est défini pour le module, un assistant manuel vous permet d'appliquer des paramètres au module.
...
C'est ici que se retrouve la gestion des groupes d'associations de votre module.
...
Ces listes sont appelées groupes d'associations et elles sont toujours liées à certains événements (par exemple le bouton
pressé, les déclencheurs de capteurs, ... ).

Dans le cas où un événement se produit, tous les périphériques enregistrés dans le groupe d'associations concerné recevront une commande Basic.

[TIP]
Voir la documentation du module, pour comprendre les différents groupes d'associations possibles et leur comportement.

[TIP]
La majorité des modules ont un groupe d'associations qui est réservé pour le contrôleur principal, il est utilisé pour remonter les informations au contrôleur. Il se nomme en général : *Report* ou *LifeLine*.
...
[IMPORTANT]
Certains modules doivent être associés à l'instance 0 du contrôleur principal afin de bien fonctionner.
...
Cette action est requise lorsqu'on vous informe qu'une mise a jour de paramètres et ou de comportement du module sont requises pour le bon fonctionnement.
...
[IMPORTANT]
Seuls les modules sur pile peuvent être supprimés via cet assistant.
