...
** Rafraîchissement de l'état après une COMMAND_CLASS_SWITCH_BINARY si aucune notification reçue.

* Bug Fixes

** Rafraîchissement de l'ensemble des instances suite à un broadcast de la CC Switch ALL.
** Retour json de la route /ZWaveAPI/Run/devices[<int:node_id>].GetPendingChanges() plus inclus dans result.
** Retour json de la route /ZWaveAPI/Run/devices[<int:node_id>].GetLastNotification() plus inclus dans result.
...
** Prise en charge de la restauration du dernier niveau connue sur les dimmers.
...
** Correction de possibles problèmes lors de la mise à jour des configurations de modules.
...
** Ne pas afficher d'avertissement de wakeup expiré pour les modules sur piles alimentés par secteur.
...
** Ajout d'une route pour gérer les multi-instance associations.
...
** Ajout d'une option pour désactiver la mise à jour automatique des configs des modules.
** Ajout d'une route pour modifier à l'exécution les traces de log du serveur REST. Note: aucun effet sur le niveau OpenZWave. http://ip_jeedom:8083/ZWaveAPI/Run/ChangeLogLevel(level) level => 40:Error, 20: Debug 10 Info
...
** Ajout de traces lors d'erreurs de notifications.
...
** Forcer une actualisation de l'état des interrupteurs et dimmers suite à l'envoi d'une commande switch all.
...
** Correction de la sélection de la documentation de modules pour les modules avec plusieurs profils.
...
** Correction d'un bug qui recrée les commandes lors de la mise à jour.
...
** Suppression automatique de nœud fantôme en un seul clic.
...
** Refactoring complet du code du serveur REST, optimisation de la vitesse de démarrage, lisibilité, respect de la convention de nommage.
** Mise à l’équerre des logs.
** Simplification de la gestion du refresh manuel 5 min avec possibilité d’appliquer sur les nœuds sur piles.
