...
http://ip_jeedom:8083/ZWaveAPI/Run/devices[<int:node_id>].GetHealth()
** Ajout d'une route pour récupérer la dernière notification d'un noeud.
http://ip_jeedom:8083/ZWaveAPI/Run/devices[<int:node_id>].GetLastNotification()
