...
)
{
	...
	while( true )
	{
		if( Init( attempts ) )
		{
			...
			waitObjects[9] = m_queueEvent[MsgQueue_Refresh];		// Refresh a Node Value.
			waitObjects[10] = m_queueEvent[MsgQueue_Query];		// Node queries are pending.
			waitObjects[11] = m_queueEvent[MsgQueue_Poll];		// Poll request is waiting.
