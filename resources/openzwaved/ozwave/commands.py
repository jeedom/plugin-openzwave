...
def send_command_zwave(_node_id, _cc_id, _instance_id, _index, _value):
	...
	if _cc_id == COMMAND_CLASS_WAKE_UP:
		...
		for value_id in globals.network.nodes[_node_id].get_values(class_id=COMMAND_CLASS_WAKE_UP): 
			if globals.network.nodes[_node_id].values[value_id].label == "Wake-up Interval":
				globals.network.nodes[_node_id].values[value_id].data = int(wake_up_time)
		return
	...
	for value_id in globals.network.nodes[_node_id].get_values(class_id=_cc_id, genre='All', type='All', readonly=False, writeonly='All'):
		if globals.network.nodes[_node_id].values[value_id].instance - 1 == _instance_id and (_index is None or globals.network.nodes[_node_id].values[value_id].index == _index):
			...
			if _cc_id == COMMAND_CLASS_SWITCH_MULTILEVEL and value <= 99:
				value_utils.prepare_refresh(_node_id, value_id, value, utils.is_motor(_node_id))
			...
			if _cc_id == COMMAND_CLASS_SWITCH_BINARY:
				worker = threading.Timer(interval=0.5, function=value_utils.refresh_switch_binary, args=(_node_id, value_id, value))
				worker.start()
			return True
