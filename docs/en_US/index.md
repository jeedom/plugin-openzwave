Description
===========


.


============


. 
. 

.



. 
. 




Security.



.


-----------------------------------------------------------------


.

:

-   

-   

-   

-   

-   

> **Tip**
>
> 
> 
> Example.



 :

-   

-   

-   

-   


---------------------------




.

![introduction01](../images/introduction01.png)


.


-------------------


. 


.

![introduction02](../images/introduction02.png)

 **** 
. 
. 
. 



.


.

. 

.

> **Tip**
>
> 
> 
> . 
> 
> . 
> . 
> 
> . 
> .

> **Tip**
>
> The **Nandwork graph**  **Rorting table**
> .

> **Tip**
>
> 
> .


-------------------------------

|  | Neighbors |  |  |
|---------------------|:------------------------:|:--------------------------------------------------:|:----------------------------------------------------------------------------------------------------------------------:|
| Controller |  |  |  |
| Stheve |  |  | .  |
|  |  |  |  |

:

-   
    Posts

-   
    réseau, sollicités or non « The maître peut byler quand il veut and à
     »

-   ,
    mais seulement une réponse aux deman «L'esctheve ne byle que si
     »

-   
    
    le Controller a prédéfini « L'esctheve East torjorrs un esctheve, mais
     »

Plugin Sandup
=======================


Configuring.

![Sandup01](../images/Sandup01.png)

. 
.
.

Dependencies
-----------



 ![Sandup02](../images/Sandup02.png)

-    **Okay** .

-    **NOk**, 
     ![Sandup03](../images/Sandup03.png)

> **Tip**
>
> 
> . 
> **** .

> **Important**
>
> 
>  **NOk**, 
> 
> Dependencies.

> **Tip**
>
> 
> .

Daemon
-----


.
![Sandup04](../images/Sandup04.png) The démon local and

Information

-   The **Status** .

-   The **Sandup** 
    .

-   The button **(To reStart** 
    .

-   The button **Stopped**, 
    .

-   The **Automatic management** 
    
    .

-   The **Thest theunch** 
    .

Log
---


.

![Sandup05](../images/Sandup05.png)


.

 **Debug** or **Info** 
.

> **Important**
>
>  **Debug** 
> 
> . 
> **Debug**  ****. 
> 
>  **Error** 
> errors.

Sandup
-------------


![Sandup06](../images/Sandup06.png)

-   **Main** :

    -   **Automatically delande excluded devices** :
        
        . 
        
        
        
        .

    -   **** :
        
        

    -   **Deactivate the backgrornd update of the drives** :
        
        .

    -   **Cycle (s)** : 
        .

    -   ** key port** : 
        . ,
        
        .

    -   **** 
         : allows
        .

    -   **Backups** : 
        

    -   **** : 
        
        
        .

        > **Tip**
        >
        > 
        > .

        > **Tip**
        >
        > 
        > .

        > **Important**
        >
        > 
        > 
        > .


:

-    : 
    .

-    :
    
    .

-   Porr un Module dont le « mapping » controls a été corrigé : the
    .

    > **Tip**
    >
    > .

 ![Sandup08](../images/Sandup08.png) si
.

> **Important**
>
>  : 
> .
> . 
> ..
> .

> **Important**
>
> ,
> **Auto**, .


-------------

![Sandup09](../images/Sandup09.png)


.

Epment Sandup
=============================


Plugin :

![appliance01](../images/appliance01.png)


 :

![appliance02](../images/appliance02.png)

> **Tip**
>
> As in many ptheces on Jeedom, pthece the morse on the far left
> brings up a ck access menu (yor can, at
> from yorr profile, always leave it visible).

> **Tip**
>
>  **Synchronize**,
> **** and **Health**, 
> fashion ****. ![appliance03](../images/appliance03.png)

Main
-------

Here yor find all the Sandup of yorr epment :

![appliance04](../images/appliance04.png)

-   **Name of epment** : .

-   **Parent object** : 
    .

-   **Category** : epment categories (it may belong to
    multiple categories).

-   **Activate** : s yorr epment active.

-   **Visible** : s it visible on the dashboard.

-   **Node Id** : . 
    .
    
    
    .

-   **Module** : 
    
    . 
    

-   **** : .

-   **Sandup** : 
    Module

-   **Assistant** : 
    

-   **Documentation** : 
    .

-   **Delande** : 
    .

> **Important**
>
> 
> . ![appliance11](../images/appliance11.png) Un
> 
> .

Commands
---------

 :

![appliance05](../images/appliance05.png)

> **Tip**
>
> 
> .

-   the name disptheyed on the dashboard

-   Icon : 
    

-   Order value : 
    
    . 
    .

-   type and subtype.

-   .

-   .

-   .

-   .

-   "Status feedback value "and" Duration before status feedback" : allows
    to indicate to Jeedom that after a change in the information
    value must randurn to Y, X min after the change.  : IN
    the case of a presence dandector which emits only during a
    presence dandection, it is useful to sand for example 0
    
    
    .

-   Historize : allows to historize the data.

-   Pin up : allows to dispthey the data on the dashboard.

-   Invert : .

-   Unit : data unit (can be empty).

-   Min / max : data bornds (may be empty).

-   Advanced Sandup (small notched wheels) : Disptheys
    the advanced Sandup of the command (mandhod
    history, widgand…).

-   TEast : Used to tEast the command.

-   Delande (sign -) : allows to delande the command.

> **Important**
>
> The button **TEast** 
> 
> . 
> 
> . 
> ,
> .

The ****, 
.
![appliance13](../images/appliance13.png) Si aucune Command n'East

.

> **Important**
>
> The **** . 
> 
> .


-----------------



![appliance06](../images/appliance06.png)


allows.

> **Important**
>
> 
> Commands.


--------------------------


.

![appliance07](../images/appliance07.png)

The button **Documentation** 
.


.

The button **Assistant** 
.

Recommended Sandup
-------------------------

![appliance08](../images/appliance08.png)


Jeedom.

> **Tip**
>
> 
> .


.

-   **Sandtings** 
    .

-   **** .

-   ****, .

-    **** 
    .


: **Recommended Sandup**, 
.

![appliance09](../images/appliance09.png)

.



![appliance10](../images/appliance10.png)

> **Important**
>
> 
> Sandup.


. 

.

![appliance11](../images/appliance11.png)

> **Tip**
>
> 
> 
> .


=========================



![node01](../images/node01.png)

 :

Summary
------



.

> **Tip**
>
> 
> 
> . 
> 
> .

Values
-------

![node02](../images/node02.png)


.
The « mapping »  Commands East entièrement basé sur ces Information.

> **Tip**
>
> . 
> . It is
> 
> Module documentation.

> **Tip**
>
> 
> . 
> .

> **Important**
>
> 
> 
> . 
> Refreshing. 
> 
> .
> ![node16](../images/node16.png) L'ensemble  valeurs (index) de
> 
> 
> . .

Sandtings
----------

![node03](../images/node03.png)



.

,
![node04](../images/node04.png) le byamètre East en attente d'être
.

.


.
![node05](../images/node05.png)


. 
.

.

> **Tip**
>
> 
> . 
> .

> **Tip**
>
>  **** 
> .

![node06](../images/node06.png)

> **Tip**
>
>  **** 
> 
> .

![node18](../images/node18.png)

> **Tip**
>
>  **Update sandtings** 
> .


.
![node17](../images/node17.png) Veillez vors référer à the documentation
.

Associations
------------


Module.

![node07](../images/node07.png)


. 
.





.



.

> **Tip**
>
> 
> .

> **Tip**
>
> 
> 
> .  : **** or
> ****.

> **Tip**
>
> .

> **Tip**
>
> 
> . 
> .

,
 **Associated with which Modules**

![node08](../images/node08.png)


.

****

.



![node09](../images/node09.png)

> **Important**
>
> 
> . 
> .

Systems
--------

.

![node10](../images/node10.png)

> **Tip**
>
> 
> . 
> 
> . 
> 
> .
> ![node11](../images/node11.png)

> **Tip**
>
>  **** and **** 
>  **** . 
> . 
> .  **** 
> .

Actions
-------

.

![node12](../images/node12.png)



.

> **Important**
>
> 
> . . The actions
> 
> .

> **Tip**
>
> The **** 
> . 
> 
> 
> 
> .

> **Tip**
>
> 
>  ****, It is
> .

![node13](../images/node13.png)

> **Tip**
>
> 
> 
> **** 
> . 
> 
> .

![node14](../images/node14.png)



.

> **Important**
>
> .

Statistics
------------

.

![node15](../images/node15.png)


".


=====================

.

Inclusion fashion
--------------


. 
**Inclusion**. .
 **Exclusion**. 
.

![addremove01](../images/addremove01.png)


.


**Inclusion**.

![addremove02](../images/addremove02.png)


. 
 **Secured**.


 **Insecure**.

 : .



'
.

![addremove03](../images/addremove03.png)



.

> **Tip**
>
> 
> Inclusion.

.

> **Tip**
>
> 
> " **** 
> . 
> 
> .

> **Important**
>
> 
> .

> **Tip**
>
> 
> **Secured**, .

> **Tip**
>
> ,
> .

> **Tip**
>
> 
> . 
> . 
> 
> .

> **Tip**
>
> 
> 
> .

Exclusion fashion
--------------

![addremove04](../images/addremove04.png)




.

![addremove05](../images/addremove05.png)

> **Tip**
>
> 
> Exclusion.

.

> **Tip**
>
> .

> **Tip**
>
> 
> . 
> .

Synchronize
------------

![addremove06](../images/addremove06.png)


. ,

,
 **Automatically delande excluded devices** East
activated.




.

> **Tip**
>
> ,
> 
> .

> **Tip**
>
> 
>  ****, the
> .

 :
![addremove07](../images/addremove07.png)


==============

![nandwork01](../images/nandwork01.png)

.

![nandwork02](../images/nandwork02.png)

Summary
------

,

.

**Information**

-   
    
    .

-   
    .

-   
    .

-   .

**State**

![nandwork03](../images/nandwork03.png)

 :

-    ****, ****
    or ****.

-   
    . 
    
    ****.

 ****, 


.

> **Tip**
>
> 
> ****, 
> . 
> 
> 
> .

 ****, 
.

> **Tip**
>
> 
>  ****. ,
> 
> . 
> 
> .

**Capacities**


.

**System**

.

-   .

-   

-   

Actions
-------

![nandwork05](../images/nandwork05.png)


. .

> **Important**
>
> 
> 
> .

> **Important**
>
> 
> . 
> .

> **Tip**
>
> 
> .

Statistics
------------

![nandwork06](../images/nandwork06.png)


.

Nandwork graph
-------------------

![nandwork07](../images/nandwork07.png)


.

 :

-   **Btheck** : 
    .

-   **Green** : .

-   **** : 
    .

-   **Yellow** : 
    .

-   **** : 
    .

-   **Red** : 
    .

> **Tip**
>
> .


.



:

Rorting table
----------------


Direct communication. . 

. 


.

![nandwork08](../images/nandwork08.png)


. 

.

 :

-   **Green** : .

-   **** : At least 2 rortes with a jump.

-   **Yellow** : Thess than 2 rortes with a jump.

-   **** : 
    .

-   **Orange** : All roads have more than one jump. 
    .

> **Tip**
>
> .

> **Important**
>
> .
> .

> **Tip**
>
> 
> 
> Rorting table.

Health
=====

![health01](../images/health01.png)

 :

![health02](../images/health02.png)

 :

-   **Module** : 
    .

-   **Id** : .

-   **Notification** : 
    Controller

-   **Grorp** : 
    . 
    

-   **Manufacturer** : 
    

-   **** : 

-   **Status** : 
    Module

-   **Drums** : 
    .

-   **Wakeup time** : 
    
    .

-   **Total package** : 
    .

-   **** : 
    .

-   **Time dandhey** : .

-   **Thest notification** : 
    
    .

    -   
        .

    -   .

-   **Ping** : 
    .

> **Important**
>
> 
> .

:

![health04](../images/health04.png) Modules supportant the


![health05](../images/health05.png) Modules supportant the
.

![health06](../images/health06.png) Modules supportant the
.

![health07](../images/health07.png) Module FLiRS, rorteurs esctheves
.

> **Tip**
>
> 
> ".

> **Tip**
>
> 
> .

> **Tip**
>
> 
> . 
>  **** 
> .

> **Tip**
>
> 
> . 
> 
> .

> **Tip**
>
> 
> 

> **Tip**
>
> 
> .

> **Tip**
>
> 
> 
> . 
> .

![health03](../images/health03.png)

> **Tip**
>
> 
>  ****, the
> .

> **Tip**
>
> 
>  ****, 
> . Yor have
>  **NOk** . 
> .
> 


---------------------

.

-   **** Initialization of the node search process.

-   **** 
    
    .

-   **** .

-   **** 
    .

-   **** 
    .

-   **** 
    .

-   **** 
    .

-   **** 
    .

-   **** 
    .

-   **** Randrieve version information.

-   **** 
    control.

-   **** .

-   **** 
    .

-   **Associations** Randrieve information on associations.

-   **** Randrieve the list of neighboring no.

-   **** .

-   **** Randrieve dynamic information
    .

-   **Sandup** 
    .

-   **** .

Notification
------------



-   **** .

-   **Timeort** .

-   **** 
    .

-   **** 

-   **** .

-   **** .

-   **** .

Backups
=======


. .

 :

-   
    . 
    

-   
    

-   

![backup01](../images/backup01.png)


=======================


. 
:

![update01](../images/update01.png)

> **Tip**
>
> 
> Plugin.


 **NOk**. 
.





. 
.

> **Tip**
>
> 
>  **** 
> .


============================


[ici](https://jeedom.fr/doc/documentation/zwave-Modules/fr_FR/doc-zwave-Modules-epement.compatible.html)


=======================


-------------------------------------------------------------------------------

![trorbleshooting01](../images/trorbleshooting01.png)


.

 **Regenerate
**  **Zwave nandwork** tab
**Actions**.


--------------------------------------------------

![trorbleshooting02](../images/trorbleshooting02.png)


.


 ****
 **Actions**.


 **Repthece failed node**
 **Actions**, 
. 
.


--------------------------------------

![trorbleshooting03](../images/trorbleshooting03.png)

. 
.


Synchronization.

![trorbleshooting04](../images/trorbleshooting04.png)


. 
.

:

-   .

-   .

-   .

-   .

.



.


----------------------------------------------

![trorbleshooting05](../images/trorbleshooting05.png)

.

 **Info**  **** Instance **0** Command
****

. 
.


-------------------------------------


.


.

![trorbleshooting06](../images/trorbleshooting06.png)

 **Action**  **** 
**Instance**  ****


up to date. 
.


.



http://token:\#APIKEY\#@ip\_jeedom:8083/ZWaveAPI/Run/devicesnode\_id.instances\[instance\_id\].commandCthesses\[cc\_id\].data\[index\].ForceRefresh()


[içi](./rEastapi.asciidoc).


------------------------------------------------


.

 ****  **** 
 **Resand** .

,
:

-   .

-   
    .

-   
    
    .

-    **Resand**, 
    .

-   
    . .

-    **0**.

-   .

-   
    .

-   
    . .

-   
    .

-   
    .

-   ,
    
    .

-   .

-   .

-   
    . .


------------------------------




" :

-   
    .

-   
    "".

-   
    Module documentation.

-   .

:

-   
    ".

-   
    .

-   .

-   .

-   .

-   .

-   .

-   
    .

-   .


----------------------------



.

.

-   .

-    ****.

-    ****.

-   . 
    File **** 
    .

-   .

-   .

-   .

-   .

-   
    .

-   
    "
    .

.


----------------------


.

.


,


. 
.

.


.

.

.

. 
.

![trorbleshooting07](../images/trorbleshooting07.png)


.


.


.

![trorbleshooting08](../images/trorbleshooting08.png)



.


-------------------------------



.




.


Cache.


. 
. 
.

> **Tip**
>
> 
> 
> 
> . 
> . 
> . ,
> . 
> 
> . 
> 
> .

> **Tip**
>
> 
> 
> 
> .


----------------------------------------



Message: 
"

> **Tip**
>
> 
> . 
> .


.

:

-   .

-   .

-   
    .

-   .
    
    

-   
    .

-    **** 
    .

-    **Debug**  **** and
    .

-   .

-   
    
    the command).

:

-   .

-   .

:

-   
    .

-   .

-   
    .

-   .


----------------------------


.

.

.

.

:

-   
    .

-   .

-   
    .

-   .


---------------------------



. ,
.

:

-   


:

    
    

-   


. 

.

-   

        : : 
        ,
        .
        :..
        error: 
        :266: 
        : *** [build] Error 1


.

.


 :

    
    
    


.

    







> **Tip**
>
>  **** by
> ****  **STOP** / **Start**. 
> :
>  .


------------------------------------------------------


.

:

    

:

    

.

HTTP API
========



HTTP.


.

:

URLs =
[http://token:\#APIKEY\#@\#IP\_JEEDOM\#:\#PORTDEMON\#/\#ROUTE\#](http://token:#APIKEY#@#IP_JEEDOM#:#PORTDEMON#/#ROUTE#)

-   
    .  «
    Main », puis « Administration » and « Sandup », en activant
    .

-   .

-   
    : 8083.

-   .


[github](https://github.com/jeedom/Plugin-openzwave) Plugin .

: 

URLs =
http://token:a1b2c3d4e5f6g7h8@192.168.0.1:8083/ZWaveAPI/Run/devices\[2\].TEastNode()

# Faq

> **"**
>
> 
